# Vibe Coding Collective with Softr 
[Start with Softr](https://studio.softr.io/dashboard "Start with Softr")

Follow us everywhere to keep an eye on new events!
[Linkedin](https://www.linkedin.com/company/vibe-coding-collective/ "Linkedin") / [Instagram](https://www.instagram.com/vibecoding.collective "Instagram") / [Twitter](https://x.com/vcodersglobal?s=21 "Twitter") / [Luma](https://luma.com/Vibecoders.global "Luma") / [Meetup](https://www.meetup.com/vibe-coding-collective/ "Meetup")

WIFI: Fritz!Box 7580RG,  
Password: 25163313676026846447

<img width="300" height="304" alt="image" src="https://github.com/user-attachments/assets/9ee00b4b-7af3-47d8-bf4c-098347bc67a0" />


## Challenges today
<img width="449" height="369" alt="image" src="https://github.com/user-attachments/assets/1d7a9d48-1aff-4523-9b18-41566c9b96fc" />



## Still no idea? no problem. steal some data! =)
Then how about using those knowledge hubs with GBs of data?
- [Kaggle: millions of CSVs, sorted by popularity](https://www.kaggle.com/datasets?sort=votes&fileType=csv_gl= "Kaggle: millions of CSVs, sorted by popularity")
- [Airtable Universe: ready-made databases from real people](https://www.airtable.com/universe?_gl= "Airtable Universe: ready-made databases from real people")  
One rule: your list needs 1000+ rows. 

## Softr Credits
FOR NEW SOFTR USERS (no existing account)
Use a referral [link](https://www.softr.io/build-with/vcc  "link") 
(This flow starts you in the AI Co-Builder, so you should generate an app first before signing up). 

FOR EXISTING SOFTR USERS (already have an account)
Use promo code **VCCBUILD** 
(Put in the code, and then start in the AI Co-Builder). 


## Useful Resources
- [Vibe Coding with Softr 101 + 5 Bonus tips](https://www.softr.io/blog/softr-building-modes-explained "Vibe Coding with Softr 101 + 5 Bonus tips")
- [How to build an app with AI (in under 10 minutes)](https://www.softr.io/blog/how-to-build-a-business-app-with-ai "How to build an app with AI (in under 10 minutes)")


##

Have something to say about our events? Leave it here: [feedback form](https://forms.gle/VHfgQbhPrsqY2Lu38 "feedback form")!

##
[vibecoders.global](https://vibecoders.global "vibecoders.global")
